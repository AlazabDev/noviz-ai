...
export const appConfig = {
  ...
  llm: {
    provider: process.env.LLM_PROVIDER || "openai", // "openai" | "azure_foundry" — see providers/llm/llmProviderFactory.ts
  ...
  // Azure AI Foundry Agent Service, reached as a plain REST API (no
  // @azure/* SDK dependency — same axios-based pattern as
  // openaiProvider.ts) via the Responses API's RESOURCE-level route
  // (".../openai/v1/responses", not the project-scoped
  // ".../api/projects/<name>/openai/v1/responses" one — the latter has a
  // known HTTP 431 bug on some resources as of late 2026, see
  // azureFoundryProvider.ts's own doc comment). Only read when
  // LLM_PROVIDER=azure_foundry.
  azureFoundry: {
    // Paste your project endpoint as-is, e.g.
    // "https://az-ai-resource.services.ai.azure.com/api/projects/az-ai-gateway"
    // — azureFoundryProvider.ts derives the resource-level endpoint from
    // it automatically (strips "/api/projects/...").
    endpoint: process.env.AZURE_FOUNDRY_ENDPOINT || "",
    agentName: process.env.AZURE_FOUNDRY_AGENT_NAME || "",
    agentVersion: process.env.AZURE_FOUNDRY_AGENT_VERSION || "",
    // Auth, option 1 (simplest — recommended to start): a Foundry
    // resource API key (Foundry portal -> your resource -> Keys and
    // Endpoint). Sent as the "api-key" header.
    apiKey: process.env.AZURE_FOUNDRY_API_KEY || "",
    // Auth, option 2 (no long-lived key stored anywhere — used only when
    // apiKey above is empty): a Microsoft Entra ID (Azure AD) app
    // registration's own client-credentials grant. azureFoundryProvider.ts
    // exchanges these for a short-lived bearer token itself over plain
    // REST (no @azure/identity dependency) and caches it until it's
    // close to expiry.
    tenantId: process.env.AZURE_FOUNDRY_TENANT_ID || "",
    clientId: process.env.AZURE_FOUNDRY_CLIENT_ID || "",
    clientSecret: process.env.AZURE_FOUNDRY_CLIENT_SECRET || "",
  },
